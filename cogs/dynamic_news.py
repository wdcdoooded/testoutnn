...
class ConfirmationView(discord.ui.View):
    ...
    async def confirm_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # 3. Visually disable the buttons so the user knows they clicked it
        for child in self.children:
            child.disabled = True

        # 4. Transform the message in-place
    ...
    async def edit_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Open the modal with pre-filled inputs
...
class NewsSetupModal(discord.ui.Modal, title="ตั้งค่าสรุปข่าวประจำวัน (Custom News Setup)"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # The Magic Trick: Check if it's an edit or a fresh command
