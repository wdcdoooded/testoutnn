    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(DB_FILE) as db:
            cursor = await db.execute(
                ...
                (interaction.user.id, interaction.channel_id, topics, time_str, days)
            )
            job_id = cursor.lastrowid
            await db.commit()

        # Add job to APScheduler
        self.cog.schedule_job(job_id, interaction.channel_id, topics, hour, minute)

        await interaction.response.send_message(
            f"✅ **บันทึกตั้งค่าสำเร็จ!**\n"
            f"📌 **หัวข้อ:** {topics}\n"
            f"⏰ **เวลา:** {time_str} น. (เวลาไทย)\n"
            f"📅 **ระยะเวลา:** {days} วัน\n"
            f"ระบบจะค้นหาข่าวเกี่ยวกับหัวข้อนี้ และส่งสรุปเข้าในช่องนี้ตามเวลาที่กำหนดไว้ครับ",
            ephemeral=False
        )
...
class DynamicNews(commands.Cog):
    ...
    async def run_custom_news_job(self, job_id, channel_id, topics):
        ...
        # Fetch articles dynamically based on the requested topics!
    ...
    def fetch_raw_news(self, topics):
        # Convert topics (e.g. "AI, Crypto") into a URL search string
        ...
        # Grab top 7 search results specifically matching the user's topics
        for entry in feed.entries[:7]:
            articles.append(f"Title: {entry.title}\nSummary: {entry.get('summary', 'No summary')}\nLink: {entry.link}")
            
        # Fallback to standard RSS if Google search returns nothing
        if not articles:
